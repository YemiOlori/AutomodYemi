...
def validate_response(func):
	...
	def wrap(*args, **kwargs):
		response = {"success": False, "error_message": "internal response - validate_response"}
		req = None
		
		try:
			req = func(*args, **kwargs)
			if isinstance(req, dict):
				req["internal_error_message"] = "clubhouse.validate_response"
				return req
			...
			if req and req.json().get("success") is False:
			...
			return req.json() if req else response
		
		except KeyError as key_error:
			logging.error(f"{func.__name__} {key_error}")
		
		else:
			response = req and req.json()
...
class Config:
	...
	def __init__(self, account, config):
		...
		self.config_file = f"/Users/deon/Documents/GitHub/automod/automod/{config}_config.ini"
		self.reload_file = "/Users/deon/Documents/GitHub/automod/automod/setting.ini"
		# self.config_object = self.load_config()
	...
	def section_key_exception(config_object, section):
		if section not in config_object.sections():
			...
			# logging.info(f"Error in fetching config in read_config method. {section} not found in config file.")
	...
	def reload_client(self):
		...
		"""
		config_object = ConfigParser()
		config_object.read(self.reload_file)
		user_config = self.config_to_dict(config_object, self.account)
		client_name = user_config.get("name")
	...
	@staticmethod
	def write_config(
			account, user_id, user_token, user_device, name,
			filename='/Users/deon/Documents/GitHub/automod/automod/setting.ini'):
...
class Auth(Config):
	...
	API_BUILD_ID = "1477"
	API_BUILD_VERSION = "22.06.28"
	API_UA = f"clubhouse/{API_BUILD_ID} (iPhone; iOS 15.5; Scale/3.00)"
	...
	def __init__(self, account, config, client_id='', user_token='', user_device='', headers=None):
		""" (Clubhouse, str, str, str, dict) -> NoneType
		Set authenticated information
		"""
		super().__init__(account, config)
		reload_dict = self.reload_client()
		self.client_name = reload_dict.get("client_name")
		self.HEADERS["CH-UserID"] = reload_dict.get("client_id")
		self.HEADERS["Authorization"] = f"Token {reload_dict.get('user_token')}"
		self.HEADERS["CH-DeviceId"] = reload_dict.get("user_device")
		...
		if not self.HEADERS.get("CH-DeviceId"):
			self.HEADERS["CH-DeviceId"] = user_device and user_device.upper() or str(uuid.uuid4()).upper()
		
		self.client_id = self.HEADERS.get("CH-UserID") and int(self.HEADERS.get("CH-UserID")) or None
...
class Clubhouse(Auth):
	...
	def __init__(self, account, config):
		Logger()
		logging.info("Initializing Clubhouse...")
		
		super().__init__(account, config)
		self.auth = Auth(account, config)
		self.client = Client(account, config)
		self.user = User(account, config)
		self.notifications = Notifications(account, config)
		self.channel = Channel(account, config)
		self.mod = ChannelMod(account, config)
		self.chat = ChannelChat(account, config)
		self.message = Message(account, config)
		self.houses = Houses(account, config)
		self.event = Event(account, config)
		self.club = Club(account, config)
		self.topic = Topic(account, config)
		self.replay = Replay(account, config)
...
class Client(Auth):
	...
	@validate_response
	def followers(self, client_id=0, page_size=50, page=1):
		""" (Clubhouse, str, int, int) -> dict

		Get followers of the given user_id.
		"""
		client_id = client_id or self.client_id
		query = "user_id={}&page_size={}&page={}".format(client_id, page_size, page)
...
class Channel(Auth):
	...
	@validate_response
	def become_speaker(self, channel):
		...
		data = {
			"channel_id": None,
			"client_leave_reason": None,
			"channel": channel,
			"source": None,
			"logging_context": None
		}
		req = requests.post(f"{self.API_URL}/become_speaker", headers=self.HEADERS, json=data)
		return req
	...
	@validate_response
	def record_external_channel_share(self, channel, share_type=1):
		""" (Clubhouse) -> dict
		Share types:
			1: Text
			2:
...
class ChannelMod(Auth):
	...
	@validate_response
	def get_channel_shares(self, channel, next_cursor=None):
		""" (Clubhouse, str, int) -> dict

		Remove the user from the channel. The user will not be able to re-join_channel.
		"""
		endpoint = f"?channel={channel}&next_cursor={next_cursor}" if next_cursor else f"?channel={channel}"
		req = requests.get(f"{self.API_URL}/get_channel_shares{endpoint}", headers=self.HEADERS)
		return req
...
class Message(Auth):
	...
	def send(self, message, chat_id=None, participant_ids=None):
		...
		# Need to create chat if one doesn't exist
...
class Houses(Auth):
	...
	def get_user_social_clubs(self):
		...
		req = requests.post(f"{self.API_URL}/get_user_social_clubs", headers=self.HEADERS, json=data)
		return req
	
	@validate_response
	def get_social_club(self, social_club_id):
		print(social_club_id)
		data = {
			"is_for_social_club_open": True,
			"social_club_id": social_club_id,
			"source": 4
		}
		req = requests.post(f"{self.API_URL}/join_social_club", headers=self.HEADERS, json=data)
		print(req)
		return req
...
class Replay(Auth):
	...
	def join_replay(self, channel):
		...
		req = requests.get(f"{self.API_URL}/join_replay_channel", headers=self.HEADERS, json=data)
		return req
