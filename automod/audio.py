...
from automod.clubhouse import Clubhouse
...
class AudioClient:
    ...
    def start_audio(self, channel, token=None, join_info=None):
        if not token and not join_info:
            join_info = Clubhouse(self.account, self.config).channel.join_channel(channel)
        ...
        if self.RTC:
            token = token
            self.RTC.joinChannel(token, channel, "", int(Clubhouse(self.account, self.config).client_id))
            self.RTC.muteAllRemoteAudioStreams(mute=True)
            self.RTC.muteLocalAudioStream(mute=False)
            Clubhouse(self.account, self.config).channel.update_audio_mode(channel)
